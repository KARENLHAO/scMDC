...
class scMultiCluster(nn.Module):
    ...
    def pretrain_autoencoder(self, X1, X_raw1, sf1, X2, X_raw2, sf2, 
        ...
        num_batch = int(math.ceil(1.0*X1.shape[0]/batch_size))
        dataset = TensorDataset(torch.Tensor(X1).float(), torch.Tensor(X_raw1).float(), torch.Tensor(
            sf1).float(), torch.Tensor(X2).float(), torch.Tensor(X_raw2).float(), torch.Tensor(sf2).float())
    ...
    def fit(self, X1, X_raw1, sf1, X2, X_raw2, sf2, y=None, lr=1., n_clusters = 4,
        ...
        X1 = torch.tensor(X1).to(self.device).float()
        X_raw1 = torch.tensor(X_raw1).to(self.device).float()
        sf1 = torch.tensor(sf1).to(self.device).float()
        X2 = torch.tensor(X2).to(self.device).float()
        X_raw2 = torch.tensor(X_raw2).to(self.device).float()
        sf2 = torch.tensor(sf2).to(self.device).float()
        self.mu = Parameter(torch.Tensor(
            n_clusters, self.z_dim), requires_grad=True)
        optimizer = optim.Adadelta(
            filter(lambda p: p.requires_grad, self.parameters()), lr=lr, rho=.95)
