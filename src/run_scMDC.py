...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--data_file', default='Normalized_filtered_BMNC_GSE128639_Seurat.h5')
    ...
    parser.add_argument('-el', '--encodeLayer', nargs='+',
                        default=[256, 64, 32, 16])
    parser.add_argument('-dl1', '--decodeLayer1',
                        nargs='+', default=[16, 64, 256])
    ...
    parser.add_argument('--f1', default=1000, type=float,
                        help='Number of mRNA after feature selection')
    parser.add_argument('--f2', default=2000, type=float,
                        help='Number of ADT/ATAC after feature selection')
    parser.add_argument('--filter1', action='store_true',
                        default=False, help='Do mRNA selection')
    parser.add_argument('--filter2', action='store_true',
                        default=False, help='Do ADT/ATAC selection')
    ...
    # get k
    latent = model.encodeBatch(torch.tensor(adata1.X).float().to(
        args.device), torch.tensor(adata2.X).float().to(args.device))
    latent = latent.cpu().numpy()
    if args.n_clusters == -1:
        n_clusters = GetCluster(
            latent, res=args.resolution, n=args.n_neighbors)
    else:
    ...
    if args.prediction_file:
        if not args.no_labels:
            ...
            np.savetxt(args.save_dir + "/" + str(args.run) +
                       "_pred.csv", y_pred_, delimiter=",")
        else:
            np.savetxt(args.save_dir + "/" + str(args.run) +
                       "_pred.csv", y_pred, delimiter=",")

    if args.embedding_file:
        final_latent = model.encodeBatch(torch.tensor(adata1.X).float().to(
            args.device), torch.tensor(adata2.X).float().to(args.device))
        final_latent = final_latent.cpu().numpy()
        np.savetxt(args.save_dir + "/" + str(args.run) +
                   "_embedding.csv", final_latent, delimiter=",")
